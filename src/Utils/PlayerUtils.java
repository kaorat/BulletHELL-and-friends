...
public class PlayerUtils {
    public static void normal(Player player){
        ...
        PlayerBullet bullet = new PlayerBullet(10, player, new Transform(player.getTransform().getPosX() + 50, player.getTransform().getPosY() + 20, -90, 2, 2), 0, PlayerManager.getInstance().getBioticRifleDamage(),1);
        PlayerBullet bullet2 = new PlayerBullet(10, player, new Transform(player.getTransform().getPosX(), player.getTransform().getPosY() + 20, -90, 2, 2), 0, PlayerManager.getInstance().getBioticRifleDamage(),1);
    ...
    public static void twin(Player player){
        // player shooting pattern
        for(int i = 0; i < 4; i++){
            PlayerBullet bullet = new PlayerBullet(10, player, new Transform(player.getTransform().getPosX() + 25 * i, player.getTransform().getPosY() + 20, -90, 2, 2), 0, PlayerManager.getInstance().getBioticRifleDamage(),1);
            BulletManager.getInstance().add(bullet);
        }
    }
    ...
    public static void autoAim(Player player){
        ...
        PlayerBullet bullet = new PlayerBullet(10, player, new Transform(player.getTransform().getPosX() + 25, player.getTransform().getPosY() + 20, rot, 2, 2), 0, PlayerManager.getInstance().getBioticRifleDamage(),1);
        BulletManager.getInstance().add(bullet);
